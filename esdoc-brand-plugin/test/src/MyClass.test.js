...
describe('test/MyClass.js:', ()=> {
  ...
  it('has brand title', ()=>{
    assert.equal($('title').text(), 'Home | esdocs-brand-plugin-test');
  });

  it('has repository link', ()=>{
    assert.equal($('header a[href="https://github.com/esdocs/esdocs-optional-plugins"]').length, 1);
  ...
  it('has meta tag', ()=>{
    // normal
    assert.equal($('meta[name="description"]').attr('content'), 'this is esdocs-brand-plugin test');

    // og
    assert.equal($('meta[property="og:type"]').attr('content'), 'website');
    assert.equal($('meta[property="og:url"]').attr('content'), 'https://esdoc.org');
    assert.equal($('meta[property="og:site_name"]').attr('content'), 'esdocs-brand-plugin-test');
    assert.equal($('meta[property="og:title"]').attr('content'), 'esdocs-brand-plugin-test');
    assert.equal($('meta[property="og:image"]').attr('content'), 'https://esdoc.org/manual/asset/image/logo.png');
    assert.equal($('meta[property="og:description"]').attr('content'), 'this is esdocs-brand-plugin test');
    ...
    assert.equal($('meta[property="twitter:title"]').attr('content'), 'esdocs-brand-plugin-test');
    assert.equal($('meta[property="twitter:description"]').attr('content'), 'this is esdocs-brand-plugin test');
